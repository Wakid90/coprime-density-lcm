...
By applying the leading-order claim~\eqref{eq:sigma-limit} with $c$ replaced by $c/r$ (the argument of Section~\ref{sec:thm4-leading} uses only the strip decomposition and applies verbatim with integer $k$ replaced by real $s = k/r$; see Remark~\ref{rem:sigma-real}), we have
\begin{equation}\label{eq:sigma-kr}
\ln M \cdot \Sigma_{k/r}(M) \longrightarrow A(c/r) \qquad (M \to \infty),
\end{equation}
for each fixed $r \geq 2$. (The integer--real discrepancy between $\lfloor cM \rfloor / r$ and $\lfloor (c/r)M \rfloor$ is at most one unit, which changes $q_p^{k/r}$ by a bounded multiplicative factor and so does not affect the limit.)
